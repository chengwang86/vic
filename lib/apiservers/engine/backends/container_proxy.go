...
func (c *ContainerProxy) StreamContainerLogs(name string, out io.Writer, started chan struct{}, showTimestamps bool, followLogs bool, since int64, tailLines int64) error {
	...
	plClient, transport := createNewAttachClientWithTimeouts(c.portlayerAddr, attachConnectTimeout, 0, attachAttemptTimeout)
...
func createNewAttachClientWithTimeouts(portlayerAddr string, connectTimeout, responseTimeout, responseHeaderTimeout time.Duration) (*client.PortLayer, *httpclient.Transport) {
...
func (c *ContainerProxy) AttachStreams(ctx context.Context, ac *AttachConfig, stdin io.ReadCloser, stdout, stderr io.Writer) error {
	...
	client, transport := createNewAttachClientWithTimeouts(c.portlayerAddr, attachConnectTimeout, 0, attachAttemptTimeout)
	defer transport.Close()
