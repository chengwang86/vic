...
func TestMigrateConfigure(t *testing.T) {
	...
	assert.Equal(t, version.MaxPluginVersion, latestVer, "upgrade version mismatch")
...
func TestIsDataOlder(t *testing.T) {
	...
	assert.Equal(t, nil, err, "should not have error")
	assert.True(t, older, "Test data should be older than latest since container update plugin has been registered")
}
